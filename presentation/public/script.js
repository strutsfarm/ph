...
const temperatureChartConfig = {
  ...
  data: {
    ...
    datasets: [
      {
        data: [],
        backgroundColor: 'rgba(241, 82, 52, 0.5)'
      }
    ]
  },
  options: {
...
const humidityChartConfig = {
  ...
  data: {
    ...
    datasets: [
      {
        data: [],
        backgroundColor: 'rgba(40, 144, 59, 0.5)'
      }
    ]
  },
  options: {
...
const fetchTemperature = () => {
  fetch('/temperature')
    ...
    .then(data => {
      ...
      pushData(temperatureChartConfig.data.labels, timeNow, 100)
      pushData(temperatureChartConfig.data.datasets[0].data, data.value, 100)
...
const fetchHumidity = () => {
  fetch('/humidity')
    ...
    .then(data => {
      ...
      const timeNow =
        now.getHours() + ':' + now.getMinutes() + ':' + now.getSeconds()
      pushData(humidityChartConfig.data.labels, timeNow, 100)
      pushData(humidityChartConfig.data.datasets[0].data, data.value, 100)
