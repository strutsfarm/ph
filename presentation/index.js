...
app.get('/temperature', function (req, res) {
  res.json({
    value: getCachedSensorReadings.getTemperature().toFixed(2)
  })
})

app.get('/temperature/history', function (req, res) {
  databaseOperations.fetchLatestReadings('temperature', 100, (err, results) => {
    if (err) {
      /**
...
app.get('/humidity/history', function (req, res) {
  databaseOperations.fetchLatestReadings('humidity', 100, (err, results) => {
    if (err) {
      console.error(err)
      return res.status(500).end()
    }
...
app.get('/humidity', function (req, res) {
  res.json({
    value: getCachedSensorReadings.getHumidity().toFixed(2)
  })
})
